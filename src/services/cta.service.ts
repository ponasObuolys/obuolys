...
export const ctaAnalyticsService = {
  ...
  async getTopCTAs(daysBack = 30, limit = 10) {
    const { data, error } = await supabase.rpc("get_top_ctas", {
      days_back: daysBack,
      limit_count: limit,
    });
  ...
  async getRecommendations() {
    const { data, error } = await supabase.rpc("get_cta_recommendations");
...
export const stickyMessageService = {
  ...
  async create(
    data: Omit<StickyMessage, "id" | "created_at" | "updated_at">
  ...
  async delete(id: string): Promise<void> {
    const { error } = await supabase.from("sticky_cta_messages").delete().eq("id", id);
