...
describe('SafeHtml Component', () => {
  ...
  it('respects custom allowed tags configuration', () => {
    ...
    expect(sanitizeMock).toHaveBeenCalledWith(content, expect.objectContaining({
  ...
  it('prevents DOM clobbering attacks', () => {
    ...
    expect(sanitizeMock).toHaveBeenCalledWith(maliciousContent, expect.objectContaining({
      SANITIZE_DOM: true
    }));
  });
