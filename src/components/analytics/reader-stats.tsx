...
export function ReaderStats({ articleId, className, variant = "default" }: ReaderStatsProps) {
  ...
  const displayLiveReaders =
    liveReaders === 0 ? 0 : liveReaders === 1 ? 1 : Math.round(liveReaders * multiplier);
    ...
    return (
      <div className={cn("flex items-center gap-4 text-sm text-muted-foreground", className)}>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-medium">{liveLoading ? "..." : displayLiveReaders}</span>
        ...
        {!statsLoading && viewCount > 0 && (
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-medium">{viewCount.toLocaleString("lt-LT")}</span>
            <span className="hidden sm:inline">peržiūrų</span>
          </div>
        )}
      </div>
    );
    ...
    >
      ...
      <div className="flex items-center gap-3 p-3 bg-green-500/10 rounded-md border border-green-500/20">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-muted-foreground">Skaito dabar</p>
          <p className="text-2xl font-bold text-foreground">
            {liveLoading ? <span className="animate-pulse">...</span> : displayLiveReaders}
          </p>
        </div>
      </div>
